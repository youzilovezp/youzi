...
#
# 数据本体(2026-08-30 外置):references/canonical-features-whatsapp.json
# —— 614 行数据即代码搬出渲染器,改清单不再动渲染逻辑;
# 加载失败(文件缺失/损坏)返回空列表 → 权威矩阵降级消失而非崩溃
# (vendor 矩阵不受影响,canonical 仅 feature_canonical.enabled=true 时消费)。
# ─────────────────────────────────────────────────────────
def _load_canonical_features() -> list:
    import json as _json

    p = (
        Path(__file__).resolve().parent
        / "references"
        / "canonical-features-whatsapp.json"
    )
    try:
...
_CANONICAL_FEATURES_WHATSAPP = _load_canonical_features()
    ...
    if isinstance(c.get("core_features"), list):
        ...
        for f in c["core_features"]:
            ...
            if key in seen_cf:
                ...
                continue
            seen_cf[key] = f
            cleaned_cf.append(seen_cf[key])
        c["core_features"] = cleaned_cf
