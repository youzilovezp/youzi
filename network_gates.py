        ...
        for key in ("gtm_evidence", "moat_evidence"):
            for i, ev in enumerate(competitor.get(key) or []):
                ...
                try:
                    ...
                    if _SSL_CTX is not None:
                        resp_ctx2 = urllib.request.urlopen(
                            req, timeout=10, context=_SSL_CTX
                        )
                    else:
                        resp_ctx2 = urllib.request.urlopen(req, timeout=10)
                    with resp_ctx2 as resp:
