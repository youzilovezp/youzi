...
**2️⃣ 网页爬取（3 个爬虫**并行** + 智能合并 — 详见 `adapters/`）**
- **firecrawl-mcp**（推荐安装）：**96% 网页覆盖 + JS 重度 + 结构化 JSON 输出 + Batch** — 业界最强爬虫。安装：`npx -y firecrawl-cli@latest init` 或自托管 Docker
- **Crawl4AI**（推荐安装，**开源免费**）：本地 Python 库，LLM-ready markdown 输出，处理 JS 重度。`pip install crawl4ai && crawl4ai-setup`。详见 `adapters/crawl4ai_scraper.py`
- **Playwright MCP**（按需）：微软官方浏览器自动化，**唯一能处理登录/交互**的场景。`npx -y @playwright/mcp@latest`
- `mcp__web-reader__webReader`：**URL → LLM-friendly markdown**，轻量替代
- `WebFetch`（内置）：仅作最终 fallback
- **统一入口**（**推荐用这个**）：`from adapters import scrape_smart` — **3 个爬虫并行跑 + 智能合并去重**，详见 `references/crawl-strategy.md`
...
### Step 2 · 深度爬取（WebFetch 并行）

对每个竞品，**并行**调用 WebFetch 抓以下 URL（按可用性 fallback）：

| 优先级 | URL | 抓什么 |
|--------|-----|--------|
| P0 | `<url>` | 落地页：slogan、定位、目标用户、核心卖点 |
| P0 | `<url>/pricing` 或 `/pricing.html` | 定价模式、套餐结构 |
| P1 | `<url>/features` 或 `/product` | 功能矩阵 |
| P1 | `<url>/customers` 或 `/case-studies` | 客户类型、典型场景 |
| P2 | `<url>/about` 或 `/company` | 公司背景、融资、规模 |
| P2 | `<url>/blog` 最新 1-2 篇 | 近况、技术方向 |
| P2 | `<url>/changelog` 或 `/release-notes` | 迭代速度信号 |
| P3 | `<url>/docs` 或 `/developers` | 技术栈线索、API 能力 |

每个竞品**实际抓到的内容**写到 `OUT_DIR/02-raw/<name>.md`（已转 markdown）。

如果某个竞品**完全抓不到**（404 / 反爬 / 全 JS 渲染），改用 WebSearch 拿它的：
- `Wikipedia: <name>`
- `Crunchbase / 36氪 profile`
- `<name> 评测 site:g2.com`

### Step 3 · 结构化分析（基于 raw 输出 JSON）

读取 `OUT_DIR/02-raw/<name>.md`，对每个竞品**按 references/analysis-framework.md 提取 13 个字段**：

1. `name` — 产品名
2. `url` — 官网
3. `tagline` — 一句话定位（≤ 25 字）
4. `founded` — 成立年份（可空）
5. `stage` — 阶段（早期 / 成长期 / 成熟期 / 巨头）
6. `target_users` — 目标用户（数组）
7. `core_features` — 核心功能（3-6 条，每条 ≤ 12 字）
8. `pricing` — 定价摘要（"免费 + 付费 $X/月起"）
9. `strengths` — 3 个最强项
10. `weaknesses` — 3 个最弱项
11. `differentiators` — 1-2 个差异化杀手锏
12. `tech_signals` — 技术栈线索（来自 docs/blog/changelog）
...
任意一项不过 → 用 Edit 修 `templates/report.html` 或 `render.py`，重跑 Step 4。
...
- **诚实**：抓不到的内容不要瞎编。strengths / weaknesses / scores 必须基于实际抓到的证据（带 URL 引注），不写"AMAZING" / "BEST" 这种空话。
